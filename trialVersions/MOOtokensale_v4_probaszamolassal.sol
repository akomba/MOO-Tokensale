...
contract MooTokenSale is Ownable,Sender {
  ...
  */
  modifier onlyAuthorised() {
    ...
    require (now >= presale_startTimestamp && now < presale_endTimestamp 
    || now >= publicsale_startTimestamp && now < publicsale_endTimestamp);
    require (!(hasEnded()));
  ...
  */
  ...
  function setRate(uint newRate) onlyCSorAdmin public {
  ...
  function buyTokens(address beneficiary, uint256 amount) onlyAuthorised internal {
    // check we are in time ( presale or public sale phase)
    
    
    

    if (now <= presale_endTimestamp) {
      rate = basicRate * 10/3 * 5/4;
    ...
    tokenRaised = tokenRaised.add(amount.mul(rate)); // so we can go slightly over
    token.mint(beneficiary, amount.mul(rate));
