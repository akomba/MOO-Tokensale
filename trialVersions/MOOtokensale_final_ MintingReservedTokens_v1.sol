...
/**
  * @dev  only Admin can send tokens by ourself
  */
  ...
  function finishSale() public onlyOwner {
    ...
    if(maxTokens > tokenRaised) {
      unassigned  = tokensForSale.sub(tokenRaised);   // till tokenForsale we mint the tokens
      token.mint(multiSig,unassigned);
    }
    //token.finishMinting();
    //token.transferOwnership(owner);
    PublicSaleClosed();
    token.startedTrading();
    mintigPerYear();
  }


    function mintigPerYear internal () {   
     uint _yearamount = 10*10**6;

     for(uint i = 1; i<21; i++){
            token.mint(multiSig,_yearamount);
        }
